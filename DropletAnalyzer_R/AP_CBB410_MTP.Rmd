...
droplet_data <- read_csv("Data/PICOINJ_3_Results.csv")
...
droplet_data <- droplet_data %>%
  mutate(diameter_um = radius_um * 2)
...
# Scatter Plot Overview: All Experiments

This scatter plot shows the relationship between droplet diameter and gray value for all experiments, with points colored by voltage level and faceted by flow parameters.

```{r scatter_all, fig.width=16, fig.height=10}
# Filter to valid droplets
valid_droplets <- droplet_data %>%
  filter(valid == "VALID")

# Scatter plot with all voltages combined
valid_droplets %>%
  ggplot(aes(x = diameter_um, y = grayval, color = factor(Voltage))) +
  geom_point(alpha = 0.3, size = 1.5) +
  facet_grid(Femulsion ~ Finject, 
             labeller = labeller(Femulsion = label_both, 
                                Finject = label_both)) +
  scale_color_manual(
    values = c("250" = "#2E86AB", "500" = "#A23B72", "750" = "#F18F01"),
    name = "Voltage (V)"
  ) +
  labs(
    title = "Droplet Characteristics Across All Voltages",
